// Floating quick actions. The + focuses the Day add input when it is
// already on screen; from other tabs App switches to Day first.

function GearIcon() {
  return (
    <svg viewBox="0 0 24 24" width="31" height="31" fill="none" stroke="currentColor" strokeWidth="2.35" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
export default function FloatingAdd({ showAdd = true, onSettings, onAddFallback }) {
  ...
  function handleClick() {
    if (focusAddInput()) return;
    if (!onAddFallback) return;
    onAddFallback();
    setTimeout(focusAddInput, 80);
    setTimeout(focusAddInput, 180);
  }
  return (
